...
def get_access_token(consumer_key, redirect_uri = REDIRECT_URI):
    ...
    request = urllib.request.Request(REQUEST_TOKEN_URL, urllib.parse.urlencode(parameters).encode('utf-8'), headers)
    ...
    request = urllib.request.Request(AUTHORIZE_REQUEST_TOKEN, urllib.parse.urlencode(parameters).encode('utf-8'), headers)
    resp = urllib.request.urlopen(request)
